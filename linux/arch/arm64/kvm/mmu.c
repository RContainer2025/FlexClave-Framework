...
{
	// 调用FC_MODIFY_PAGE_PERM，传入要解映射的IPA范围
...
{
	// 初始化整个S2 MMU，kvm_pgtable_stage2_init() will allocate the PGD, should be managed by FLEXCLAVE
...
{
	// 销毁S2页表，应该也需要进入FLEXCLAVE做kvm_init_stage2_mmu之后的一致的销毁操作
	struct kvm *kvm = kvm_s2_mmu_to_kvm(mmu);
	struct kvm_pgtable *pgt;
...
{
	// 写保护 stage 2 页表条目，需调用FC_MODIFY_PAGE_PERM，传入IPA范围，并将attributes中的写权限位清除
	struct kvm *kvm = kvm_s2_mmu_to_kvm(mmu);
...
{
	// Copy-on-Write，调用FC_MODIFY_PAGE_PERM，传入单个IPA和新的HPA
