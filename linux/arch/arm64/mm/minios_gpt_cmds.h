#ifndef __MINIOS_GPT_CMDS_H
#define __MINIOS_GPT_CMDS_H
...
static inline __NEWSPACE unsigned long gpt_get_gptbrel3(void)
{
...
static inline __NEWSPACE unsigned long gpt_set_gptbrel3(unsigned int index)
{
	struct arm_smccc_res res;
...
static inline __NEWSPACE unsigned long gpt_read_any_phy_addr(phys_addr_t phys)
{
	struct arm_smccc_res res;
...
static inline __NEWSPACE unsigned long gpt_set_gpi(phys_addr_t phys,int pagecount,unsigned int gpi)
{
	struct arm_smccc_res res;
...
static unsigned int gpt_os_init;
// 记录对应index下的GPT是否已经被使用，0表示未使用，1表示已经被使用，注意gpt_used[0]永远为1，因为0号GPT是OS所在的GPT
static int gpt_used[GPT_DOCKER_COUNT_MAX + 1] = {0};

// 函数调试输出gpt_used
static inline __NEWSPACE void print_gpt_used(void)
{
	...
	{
		// printk("%d ",gpt_used[i]);
		sprintf(buf,"%s%d ",buf,gpt_used[i]);
	}
	printk("gpt_used:%s\n",buf);
}

// 函数查找一个未被使用的GPT index，返回值为该GPT index，如果没有未被使用的GPT index，则返回-1
static inline __NEWSPACE unsigned int find_unused_gpt(void)
{
	int i;
