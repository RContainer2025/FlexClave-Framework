...
static const gpt_t_val_e gpt_t_lookup[] = {PPS_4GB_T, PPS_64GB_T,
					   ...
					   PPS_4PB_T};
static uintptr_t muti_gpt_l0_base_array[32] = {0};
static int muti_gpt_initialized = 0;
/*
 * Lookup P from PGS
 *
 *   PGS    Size    P
...
int gpt_set_gpi(uint64_t base,size_t size,unsigned int gpi)
{
	gpi_info_t gpi_info;
	uint64_t nse;
	int res;
...
void muti_gpt_init()
{
	//GPI_ROOT = 0xA
	//eable new_gpt_mem
	uint64_t new_gpt_mem=0xC0000000;
	for(int64_t offest=0;offest<0x2000000;offest+=PAGE_SIZE)
		gpt_set_gpi(new_gpt_mem+offest,PAGE_SIZE,0xA);

	gpt_l1_tbl=0xC0000000;
	gpt_next_l1_tbl_idx=0;

	uint64_t base =new_gpt_mem;
	gpt_next_l1_tbl_idx++;

	for(int i=0;i<32;i++)
	{
		muti_gpt_l0_base_array[i]=(uintptr_t)(base+PAGE_SIZE*i);
		copy_current_gpt((void* )(muti_gpt_l0_base_array[i]));
		//*(int*)muti_gpt_l0_base_array[i]=0xff;
	}

	muti_gpt_initialized=1;
}

int copy_current_gpt(void* new_base)
{
	void* old_l0_base = (void*)(read_gptbr_el3()<<12);
	void* new_l0_base = new_base;
	memcpy((void*)(new_l0_base),(void*)(old_l0_base),64*0x8);

	for(int i=0;i<64;i++)
	{
		uint64_t level0_value;
		level0_value=*(uint64_t* )(old_l0_base+0x8*i);
		// level 0 GPT Block
		if((level0_value&0xf)==0x1)
		{
			((uint64_t *)(new_l0_base))[i]=level0_value;
		}        
		else //level 0 GPT Table
		{

			uint64_t new_l1_base=gpt_l1_tbl+32*PAGE_SIZE*gpt_next_l1_tbl_idx;        
			((uint64_t *)(new_l0_base))[i]=new_l1_base|0x3;
			gpt_next_l1_tbl_idx++;

			uint64_t old_l1_base=(level0_value>>12)<<12;
			memcpy((void*)new_l1_base,(void*)old_l1_base,32*PAGE_SIZE);
...
int set_gpt(uint64_t index)
{	
	if(muti_gpt_initialized==0)	muti_gpt_init();

	/* Write the base address of the L0 tables into GPTBR */
	gpt_config.plat_gpt_l0_base=muti_gpt_l0_base_array[index];	
	write_gptbr_el3(((gpt_config.plat_gpt_l0_base >> GPTBR_BADDR_VAL_SHIFT)
			>> GPTBR_BADDR_SHIFT) & GPTBR_BADDR_MASK);
	//gpt_runtime_init();

	//uintptr_t* virtaddr=0;
	//mmap_add_region_alloc_va(l0_base,(uintptr_t)l0_base,0x1000,0x1);
	//mmap_add_dynamic_region(l0_base,l0_base,PAGE_SIZE,MT_RO_DATA);

	//mmap_add_region(l0_base,l0_base,PAGE_SIZE,MT_RO_DATA);
	//return muti_gpt_l0_base_array[0];
	return 0;
}